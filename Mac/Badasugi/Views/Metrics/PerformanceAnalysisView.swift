...
struct PerformanceAnalysisView: View {
    ...
    private var systemInfoSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(spacing: 12) {
                SystemInfoCard(label: "기기", value: getMacModel())
                SystemInfoCard(label: "프로세서", value: getCPUInfo())
                SystemInfoCard(label: "메모리", value: getMemoryInfo())
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 16) {
                ...
                HStack {
                    ...
                    MetricDisplay(
                        title: "평균 처리 시간",
                        value: String(format: "%.2f초", modelStat.avgProcessingTime),
                        color: .teal
                    )
                }
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .center) {
                Text(String(format: "%.2f초", modelStat.avgProcessingTime))
                    .font(.system(size: 24, weight: .bold, design: .rounded))
