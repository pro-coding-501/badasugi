...
struct AudioTranscribeView: View {
    ...
    var body: some View {
        ...
        .alert("오류", isPresented: .constant(transcriptionManager.errorMessage != nil)) {
    ...
    private var dropZoneView: some View {
        VStack(spacing: 16) {
            if isAudioFileSelected {
                VStack(spacing: 16) {
                    Text("오디오 파일 선택됨: \(selectedAudioURL?.lastPathComponent ?? "")")
                        .font(.headline)
                    ...
                    if let enhancementService = whisperState.getEnhancementService() {
                        VStack(spacing: 16) {
                            ...
                            HStack(spacing: 16) {
                                Toggle("AI 향상", isOn: $isEnhancementEnabled)
                                    .toggleStyle(.switch)
            ...
            Text("지원 형식: WAV, MP3, M4A, AIFF, MP4, MOV")
                .font(.caption)
                .foregroundColor(.secondary)
        }
        .padding()
