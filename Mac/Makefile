...
# Run application
run:
	@echo "Looking for Badasugi.app..."
	@APP_PATH=$$(find "$$HOME/Library/Developer/Xcode/DerivedData" -name "Badasugi.app" -type d | head -1) && \
	if [ -n "$$APP_PATH" ]; then \
		echo "Found app at: $$APP_PATH"; \
		open "$$APP_PATH"; \
	else \
		echo "Badasugi.app not found. Please run 'make build' first."; \
		exit 1; \
	fi
...
help:
	...
	@echo "  setup              Copy whisper XCFramework to Badasugi project"
	@echo "  build              Build the Badasugi Xcode project"
	@echo "  run                Launch the built Badasugi app"
