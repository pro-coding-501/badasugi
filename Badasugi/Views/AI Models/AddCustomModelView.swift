...
struct AddCustomModelCardView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !isExpanded {
                Button(action: {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text(editingModel != nil ? "모델 편집" : "모델 추가")
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 20) {
                    // Header
                    HStack {
                        Text(editingModel != nil ? "사용자 지정 모델 편집" : "사용자 지정 모델 추가")
                            .font(.headline)
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        FormField(title: "표시 이름", text: $displayName, placeholder: "내 사용자 지정 모델")
                        FormField(title: "API 엔드포인트", text: $apiEndpoint, placeholder: "https://api.example.com/v1/audio/transcriptions")
                        FormField(title: "API 키", text: $apiKey, placeholder: "your-api-key", isSecure: true)
                        FormField(title: "모델 이름", text: $modelName, placeholder: "whisper-1")
                        
                        Toggle("다국어 모델", isOn: $isMultilingual)
                    }
                    ...
                    HStack(spacing: 12) {
                        ...
                        Button(action: {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text(editingModel != nil ? "모델 업데이트" : "모델 추가")
                                    .font(.system(size: 13, weight: .medium))
                            }
                            .foregroundColor(.white)
                            .frame(maxWidth: .infinity)
        ...
        .alert("유효성 검사 오류", isPresented: $showingAlert) {
            Button("확인") { }
        } message: {
