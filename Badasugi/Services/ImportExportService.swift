...
struct BadasugiExportedSettings: Codable {
    let version: String
...
class ImportExportService {
    ...
    func exportSettings(enhancementService: AIEnhancementService, whisperPrompt: WhisperPrompt, hotkeyManager: HotkeyManager, menuBarManager: MenuBarManager, mediaController: MediaController, playbackController: PlaybackController, soundManager: SoundManager, whisperState: WhisperState) {
        ...
        let exportedSettings = BadasugiExportedSettings(
        ...
        do {
            ...
            savePanel.nameFieldStringValue = "받아쓰기_Settings_Backup.json"
            savePanel.title = "받아쓰기 설정 내보내기"
        ...
        DispatchQueue.main.async {
            if openPanel.runModal() == .OK {
                ...
                do {
                    ...
                    let importedSettings = try decoder.decode(BadasugiExportedSettings.self, from: jsonData)
                    ...
                    if let general = importedSettings.generalSettings {
                        ...
                        // recorderType is now always .mini, no need to import
    ...
    private func showRestartAlert(message: String) {
        DispatchQueue.main.async {
            ...
            alert.informativeText = message + "\n\n중요: AI 향상 기능을 사용 중이었다면 향상 섹션에서 API 키를 다시 구성하세요.\n\n모든 변경사항이 완전히 적용되도록 받아쓰기를 재시작하는 것이 좋습니다."
            alert.alertStyle = .informational
            alert.addButton(withTitle: "OK")
